    ...
    <v-row class="pl-6 ">
      <v-col :cols="$vuetify.breakpoint.width > 1510 ? 5 : 6" class="pr-0 pb-0">
        <v-row
          ...
          justify="start"
        >
          <h3 class="title_top_size">{{ $t("campaigns.wacarts1.headerTxtSecond") }}</h3>
        </v-row>
        ...
        <v-row style="height: 24%;width: 100%" justify="start" align="center" class="pl-6 pt-0 pb-10" >
...
<!--        </v-switch>-->
    ...
    <v-expand-transition>
      ...
      >
        ...
        <v-row style=" width:100%" class="ml-4 pl-3 pr-3  py-0 my-0">
          <v-col cols="12" class="pl-0 ml-0  py-0 mb-0 mt-1">
            <span class="smsNightTimeHint">
              {{ $t("campaigns.waLanguageSelHint")}}
            </span>
          </v-col>
        </v-row>

        <v-row  justify="start" align="center" class="ml-4 pl-3 pt-0  mt-4" >
          <span class="msgCountUnder">

            {{$t("campaigns.whatsAppCampaigns.titleDiscCoupon")}}
          </span>
        </v-row>

        <v-col   class="ml-4 pl-2  my-0 py-0" cols="12">
          <v-checkbox
            v-model="waDiscountCupon"
            class="mt-1"
          >
            <template slot="label">
              <span :class="waDiscountCupon ? 'activeDiscountCupon' : 'inDiscountCupon'">
                 {{$t("campaigns.whatsAppCampaigns.checkboxDiscCoupon")}}
              </span>
            </template>
          </v-checkbox>
        </v-col>
          ...
          <v-row class="py-0 ml-4">
            <v-col cols="12 "  class="px-0 py-0">
              <v-row  class="py-0 par_disc_fields wdg_disc_codes">
                <v-col cols="6" class="py-0 mb-0 pr-6">
                  <v-text-field
...
export default {
  ...
  computed: {
    ...
    cart1Edit() {
      return this.$store.getters.getActiveTab == "abndndcrt2"
      // ||   this.$store.getters.getCart2Active
        ? true
        : false;
    },
    swtchDisabled() {
